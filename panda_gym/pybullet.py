...
class PyBullet:
    ...
    def render(
        ...
        if mode == "rgb_array":
            ...
            for ind, pos in enumerate(['front','right_shoulder','left_shoulder']):
                ...
                proj_matrix = self.physics_client.computeProjectionMatrixFOV(
                    fov=40, aspect=float(width) / height, nearVal=0.01, farVal=3.5
                )
                (_, _, px, depth, mask) = self.physics_client.getCameraImage(
                    width=width,
                    height=height,
                ...
                rgb_cams[pos] = px
                depth_cams[pos] = depth
                mask_cams[pos] = mask
                cam_misc[pos] = {'view': np.array(view_matrix).reshape(-1,4), \
                 'proj': np.array(proj_matrix).reshape(-1,4), \
                 'near': 0.01, 'far': 3.5, 'width': width, 'height': height}

            rgb_wrist, depth_wrist, mask_wrist, cam_wrist = self.render_gripper(width = width, height = height)
            rgb_cams['wrist'] = rgb_wrist
            depth_cams['wrist'] = depth_wrist
            mask_cams['wrist'] = mask_wrist
            cam_misc['wrist'] = cam_wrist

            return rgb_cams, depth_cams, mask_cams, cam_misc
    ...
    def render_gripper(
        ...
        com_p, com_o, _, _, _, _ = self.physics_client.getLinkState(self._bodies_idx['panda'], 8)
        rot_matrix = self.physics_client.getMatrixFromQuaternion(com_o)
        ...
        view_matrix = self.physics_client.computeViewMatrix(com_p, com_p + 0.1 * camera_vector, up_vector)
        ...
        cam = {'view': np.array(view_matrix).reshape(-1,4), \
                 'proj': np.array(proj_matrix).reshape(-1,4), \
                 'near': 0.01, 'far': 3.5, 'width': width, 'height': height}

        return px, depth, mask, cam
...
class PyBullet:
    ...
    def render(
        ...
        if mode == "rgb_array":
            ...
            def view_mat_peract(name):
                if name=='front':
                    vm = np.array([-1.16849501e-07, -4.22617918e-01,  9.06307948e-01,  0.00000000e+00,
                        1.00000000e+00, -5.63571533e-07, -1.31264604e-07,  0.00000000e+00,
                        6.03176615e-07,  9.06307948e-01,  4.22617948e-01,  0.00000000e+00,
                       -7.62316870e-07, -6.07861351e-01, -2.43503584e+00,  1.00000000e+00])
                elif name == 'left_shoulder':
                    vm = np.array([-1.73648244e-01,  8.92538707e-01, -4.16198031e-01,  0.00000000e+00,
                       -9.84807705e-01, -1.57378574e-01,  7.33871044e-02,  0.00000000e+00,
                        1.72924167e-07,  4.22618592e-01,  9.06307539e-01,  0.00000000e+00,
                        6.23838620e-02, -1.13591408e-01, -2.13171875e+00,  1.00000000e+00])
                elif name == 'right_shoulder':
                    vm = np.array([ 1.73648246e-01,  8.92538667e-01, -4.16198040e-01,  0.00000000e+00,
                       -9.84807673e-01,  1.57378857e-01, -7.33869073e-02,  0.00000000e+00,
                        8.94883004e-08,  4.22618555e-01,  9.06307569e-01,  0.00000000e+00,
                       -6.23848293e-02, -1.13590829e-01, -2.13171163e+00,  1.00000000e+00])
                else: 
                    return np.zeros(16)
                return vm
            ...
            for ind, pos in enumerate(['front','right_shoulder','left_shoulder']):
                ...
                view_matrix = view_mat_peract(pos)
    ...
    def render_gripper(
        ...
        camera_vector = rot_matrix.dot(init_camera_vector)
        up_vector = rot_matrix.dot(init_up_vector)
        view_matrix = self.physics_client.computeViewMatrix(com_p, com_p + 0.01 * camera_vector, up_vector)
